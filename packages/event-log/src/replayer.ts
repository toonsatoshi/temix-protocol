...
export class Replayer {
  private static applyPayload(state: CanonicalState, payload: MutationEvent['payload'], hash: string): CanonicalState {
    switch (payload.type) {
      case 'project_create':
      ...
      case 'tact_delta':
        ...
        return {
          ...
          fileTree: [
            ...
            { path: payload.path, content: payload.delta, hash },
          ],
        };
      ...
      case 'conflict_resolution':
        return {
          ...
          fileTree: [
            ...state.fileTree.filter((f) => f.path !== payload.path),
            { path: payload.path, content: payload.resolvedContent, hash },
          ],
        };
      ...
      case 'batch':
        return payload.events.reduce(
          (nextState, nestedPayload) => this.applyPayload(nextState, nestedPayload, hash),
          state
        );
  ...
  static applyEvent(state: CanonicalState, event: MutationEvent): CanonicalState {
    return this.applyPayload(state, event.payload, event.hash);
  }
}
